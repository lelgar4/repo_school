...
#   Returns true if this list changed as a result of the call
    def removeAll(self, otherList):
        initSize = self.getSize()
        for otherNode in otherList:
            for selfNode in self:
                if otherNode.element == selfNode.element:
                    self.removeAt(self.indexOf(otherNode.element))
...
#   Returns true if this list changed as a result of the call
    def retainAll(self, otherList):
        initSize = self.getSize()
        for node in otherList:
            if self.indexOf(node.element) == -1:
                self.removeAt(self.indexOf(node.element))
